...
- `启用自动续写`：开启自动续写。
- `后台续写时弹出提示`：后台继续调用 API 时弹出提示。
- `显示悬浮小圆点`：显示一个可拖动的小圆形开关。亮起表示自动续写开启，变暗表示自动续写关闭；点击小圆点可以快速开关自动续写。
- `结尾词`：只有回复末尾出现这个文本，才会停止后台续写。默认是 `[[[ST_AUTO_CONTINUE_END_9QK7V2]]]`，你也可以改成 `正文结束`、`1 2 3 4 5` 等任意文本。
- `续写触发方式`：
  - `缺少结尾词`：推荐模式。只要流式回复结束时没有结尾词，并且不是你手动停止，就继续调用。
  - `疑似 token 上限截断`：保守模式。只有明确像 token 上限截断时才继续。
- `单条回复最大调用次数`：单条回复最多允许后台续写几次，防止无限调用。后台续写卡住后重新调用，也会占用一次调用次数。
- `后台续写卡住自动重试`：后台续写请求发出后，如果长时间没有任何可见正文，就自动中断这次请求并重新调用。
- `无正文超时秒数`：自己设置等待多久还没出正文就重试，范围是 15 到 600 秒，默认推荐 120 秒。这个检测只看“第一段正文有没有出来”；如果已经开始出字，中途停顿不会被当成卡住重试。
- `最大卡住重试次数`：只限制“后台续写无正文超时”触发的重新调用，范围是 0 到 10 次，默认推荐 2 次。填 0 等于不重试卡住调用。
- `续写提示词`：后台续写时发送给模型的提示词。可以用 `{{endMarker}}` 插入你设置的结尾词。
...
建议把结尾词设置成不容易在提示词或正文中自然出现的完整标记，例如 `[[[ST_AUTO_CONTINUE_END_9QK7V2]]]`。像 `正文结束` 这类常见短语也可以用，但必须真的出现在最终输出末尾才会停止。
...
```text
[[[ST_AUTO_CONTINUE_END_9QK7V2]]]
```

建议在你的预设 / 作者要求 / 输出格式里明确写上：
...
不要把这个结尾词放在正文中间、思维链中间、示例中间或普通段落里。插件只应该在“真正完成整条回复”时看到它。

如果你已经安装过旧版本，扩展不会强制覆盖你原来的设置。请到扩展面板里手动把 `结尾词` 改成：
...
1. 打开浏览器开发者工具，查看 Console 是否有 `Claude 自动续写：设置模板加载失败`。
2. 确认扩展目录里存在 `index.js`、`manifest.json`、`settings.html` 三个文件。
3. 确认 SillyTavern 的第三方扩展列表里已经启用 `Claude 自动续写`。
...
如果只是暂时不用，可以在扩展设置里关闭 `启用自动续写`，这样就不会再给请求附加自动续写参数。

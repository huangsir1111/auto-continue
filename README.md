...
- `Show floating toggle`：显示一个可拖动的小圆形开关。亮起表示自动续写开启，变暗表示自动续写关闭；点击小圆点可以快速开关自动续写。
- `End marker`：结尾词。只有回复末尾出现这个文本，才会停止后台续写。默认是 `[[[ST_AUTO_CONTINUE_END_9QK7V2]]]`，你也可以改成 `正文结束`、`1 2 3 4 5` 等任意文本。
- `Continue trigger`：
...
如果你已经安装过旧版本，扩展不会强制覆盖你原来的设置。请到扩展面板里手动把 `End marker` 改成：
...
请先确认扩展已经更新到 `1.0.18` 或更高版本，然后刷新浏览器页面。
